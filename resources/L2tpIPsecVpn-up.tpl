...
{{#CONN_SECTION}}  "{{IPPARAM}}" )
    ...
    ${IPFW} -t mangle -F
    ${IPFW} -t mangle -A OUTPUT -m set --match-set vpn dst -j MARK --set-mark 8
    ${IPFW} -t mangle -A INPUT -m set --match-set vpn dst -j MARK --set-mark 8
    /bin/ip route add table vpn default dev ${PPP_IFACE}
    /bin/ip rule add fwmark 8 table vpn
